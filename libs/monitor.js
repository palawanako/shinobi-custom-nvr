...
module.exports = function (s, config, lang) {
    s.initiateMonitorObject = function (e) {
        ...
        if (!s.group[e.ke].mon[e.mid].streamIn) {
            s.group[e.ke].mon[e.mid].streamIn = {};
        }
        if (!s.group[e.ke].mon[e.mid].emitterChannel) {
            s.group[e.ke].mon[e.mid].emitterChannel = {};
        }
        if (!s.group[e.ke].mon[e.mid].mp4frag) {
            s.group[e.ke].mon[e.mid].mp4frag = {};
        }
        if (!s.group[e.ke].mon[e.mid].firstStreamChunk) {
            s.group[e.ke].mon[e.mid].firstStreamChunk = {};
        }
        if (!s.group[e.ke].mon[e.mid].contentWriter) {
            s.group[e.ke].mon[e.mid].contentWriter = {};
        }
        if (!s.group[e.ke].mon[e.mid].childNodeStreamWriters) {
            s.group[e.ke].mon[e.mid].childNodeStreamWriters = {};
        }
        if (!s.group[e.ke].mon[e.mid].eventBasedRecording) {
            s.group[e.ke].mon[e.mid].eventBasedRecording = {};
        }
        ...
        if (!s.group[e.ke].mon[e.mid].parsedObjects) {
            s.group[e.ke].mon[e.mid].parsedObjects = {};
        }
        if (!s.group[e.ke].mon[e.mid].isStarted) {
            s.group[e.ke].mon[e.mid].isStarted = false;
        }
    ...
    s.getMonitorCpuUsage = function (e, callback) {
        if (s.group[e.ke].mon[e.mid].spawn) {
            var getUsage = function (callback2) {
                fs.readFile(
                    '/proc/' + s.group[e.ke].mon[e.mid].spawn.pid + '/stat',
                    function (err, data) {
                        if (!err) {
                            ...
                            clearInterval(
                                s.group[e.ke].mon[e.mid].getMonitorCpuUsage,
                            );
                        }
                    },
                );
    ...
    s.getRawSnapshotFromMonitor = function (monitor, options, callback) {
        ...
        checkExists(localStream + 's.jpg', function (success) {
            if (success === false) {
                checkExists(
                    localStream + 'detectorStream.m3u8',
                    function (success) {
                        if (success === false) {
                            checkExists(
                                localStream + 's.m3u8',
    ...
    s.mergeDetectorBufferChunks = function (monitor, callback) {
        ...
        var copiedItems = [];
        var videoLength =
            s.group[monitor.ke].mon_conf[monitor.id].details
                    ...
                    exec(cat, function () {
                        var merger = spawn(
                            ...
                            s.splitForFFPMEG(
                                '-re -i ' +
                                    allts +
                                    ' -acodec copy -vcodec copy -t 00:00:' +
                                    videoLength +
                                    ' ' +
                                    pathDir +
                                    mergedFile,
        ...
        streamDirItems.forEach(function (filename) {
            if (
                filename.indexOf('detectorStream') > -1 &&
                filename.indexOf('.m3u8') === -1
            ) {
                items.push(filename);
            }
        });
        ...
        items.forEach(function (filename) {
            try {
                ...
                var tempWriteStream = fs.createWriteStream(
                    pathDir + tempFilename,
                );
    ...
    s.mergeRecordedVideos = function (videoRows, groupKey, callback) {
        ...
        var mergedFile = [];
        var mergedFile2 = [];
        //create list of video to merge
        var listFiles = '';
        videoRows.forEach(function (video) {
            var filepath =
                s.getVideoDirectory(video) +
                s.formattedTime(video.time) +
                '.' +
                video.ext;
            if (
                ...
                listFiles += 'file ' + filepath + '\n';
                items.push(filepath);
            }
        });
        //get start and end time of videos for name of merged file
        mergedFile2.push(s.formattedTime(videoRows[0].time));
        mergedFile2.push(s.formattedTime(videoRows[videoRows.length - 1].time));

        mergedFile.sort();
        mergedFile2.sort();
        mergedFile = mergedFile.join('_') + '.mp4';
        mergedFile2 = mergedFile2.join('_') + '.mp4';
        var nameMerged = '/storage/' + mergedFile2;
        var filename = mergedFile2;
        var mergedFilepath = pathDir + mergedFile;
        var mergedRawFilepath = pathDir + 'raw_' + mergedFile;
        fs.writeFileSync(pathDir + 'list.txt', listFiles);
        items.sort();
        fs.stat(nameMerged, function (err, stats) {
            if (err) {
                //not exist
                ...
                exec('sh ' + tempScriptPath, function () {
                    ...
                    merger.on('close', function () {
                        ...
                        setTimeout(function () {
                            fs.stat(nameMerged, function (err, stats) {
                                if (!err) s.file('delete', nameMerged);
                            });
                        }, 1000 * 60 * 60 * 24);
                        delete merger;
                        callback(nameMerged, mergedFile);
                    });
                });
            } else {
                //file exist
    ...
    s.cameraDestroy = function (x, e, p) {
        if (
            ...
            if (s.group[e.ke].mon[e.id].spawn) {
                ...
                if (s.group[e.ke].mon[e.id].p2p) {
                    s.group[e.ke].mon[e.id].p2p.unpipe();
                }
            ...
            if (s.group[e.ke].mon[e.id].mp4frag) {
                var mp4FragChannels = Object.keys(
                    s.group[e.ke].mon[e.id].mp4frag,
                );
            ...
            if (s.group[e.ke].mon[e.id].childNode) {
                s.cx(
                    {f: 'kill', d: s.cleanMonitorObject(e)},
    ...
    s.cameraCheckObjectsInDetails = function (e) {
        ...
        [
            ...
            if (e.details && e.details[v]) {
                try {
                    ...
                    switch (v) {
                        case 'cords':
                            s.group[e.ke].mon[e.id].parsedObjects[v] =
                                Object.values(s.parseJSON(e.details[v]));
                            break;
                        default:
                            s.group[e.ke].mon[e.id].parsedObjects[v] =
                                s.parseJSON(e.details[v]);
                            break;
                    }
                } catch (err) {}
            }
        ...
        ['stream_channels', 'input_maps'].forEach(function (v) {
            if (
                e.details &&
                e.details[v] &&
                e.details[v] instanceof Array === false
            ) {
                try {
    ...
    s.cameraControl = function (e, callback) {
        ...
        if (
            !monitorConfig.details.control_url_method ||
            monitorConfig.details.control_url_method === ''
        ) {
            monitorConfig.details.control_url_method = 'GET';
        }
        var controlURL =
            e.base + monitorConfig.details['control_url_' + e.direction];
        var controlURLOptions = s.cameraControlOptionsFromUrl(
            controlURL,
            monitorConfig,
        );
        if (
            monitorConfig.details.control_url_stop_timeout === '0' &&
            monitorConfig.details.control_stop === '1' &&
        ...
        if (monitorConfig.details.control_url_method === 'ONVIF') {
            try {
                var move = function (device) {
                    ...
                    switch (e.direction) {
                        ...
                        default:
                            ...
                            controlOptions.Velocity[direction[1]] =
                                direction[0];
                            ...
                            if (monitorConfig.details.control_stop == '1') {
                                device.services.ptz
                                    .continuousMove(controlOptions)
                                    .then(function (err) {
                                        s.userLog(e, {
                                            type: 'Control Trigger Started',
                                        });
                                        if (
                                            monitorConfig.details
                                            ...
                                            setTimeout(function () {
                                                ...
                                                .details.control_url_stop_timeout);
                                        }
                                    })
                                    .catch(function (err) {
                                        console.log(err);
                ...
                if (
                    !s.group[e.ke].mon[e.id].onvifConnection ||
                    !s.group[e.ke].mon[e.id].onvifConnection.current_profile ||
                    !s.group[e.ke].mon[e.id].onvifConnection.current_profile
                        .token
                ) {
                    s.group[e.ke].mon[e.id].onvifConnection =
                        new onvif.OnvifDevice({
                            xaddr:
                                'http://' +
                                controlURLOptions.host +
                                ':' +
                                controlURLOptions.port +
            ...
            var stopCamera = function () {
                var stopURL =
                    e.base +
                    monitorConfig.details[
                        'control_url_' + e.direction + '_stop'
                    ];
                var options = s.cameraControlOptionsFromUrl(
                    stopURL,
                    monitorConfig,
                );
                var requestOptions = {
    ...
    s.cameraControlOptionsFromUrl = function (e, monitorConfig) {
        ...
        if (
            monitorConfig.details.control_url_method === 'ONVIF' &&
            monitorConfig.details.control_base_url === ''
        ) {
    ...
    s.resetRecordingCheck = function (e) {
        ...
        s.group[e.ke].mon[e.id].recordingChecker = setTimeout(function () {
            if (
                ...
                s.group[e.ke].mon_conf[e.id].mode === 'record'
            ) {
        ...
        e.captureOne = function (f) {
            s.group[e.ke].mon[e.id].recordingSnapRequest = request(
                ...
                .on('data', function (d) {
                    ...
                    if (d[d.length - 2] === 0xff && d[d.length - 1] === 0xd9) {
                        ...
                        if (
                            ...
                            s.group[e.ke].mon[e.id].spawn.stdin
                        ) {
                            s.group[e.ke].mon[e.id].spawn.stdin.write(
                                e.buffer0,
                            );
                        }
                        if (s.group[e.ke].mon[e.id].isStarted === true) {
                            s.group[e.ke].mon[e.id].recordingSnapper =
                                setTimeout(function () {
                                    e.captureOne();
                                }, 1000 / capture_fps);
                        }
                ...
                .on('error', function (err) {
                        ...
                        switch (err.code) {
                            ...
                            case 'ETIMEDOUT':
                                ...
                                if (
                                    ...
                                    s.userLog(e, {
                                        type: lang['Fatal Maximum Reached'],
                                        msg: {
                                            code: 'ESOCKETTIMEDOUT',
                                            msg: lang.FatalMaximumReachedText,
                                        },
                                    });
                                    s.camera('stop', e);
                                } else {
                                    s.userLog(e, {
                                        type: lang['Restarting Process'],
                                        msg: {
                                            code: 'ESOCKETTIMEDOUT',
                                            msg: lang.FatalMaximumReachedText,
                                        },
                    ...
                    if (
                        e.details.fatal_max !== 0 &&
                        e.errorCount > e.details.fatal_max
                    ) {
                        clearTimeout(s.group[e.ke].mon[e.id].recordingSnapper);
                        s.launchMonitorProcesses(e);
                    }
    ...
    s.createCameraFfmpegProcess = function (e) {
        ...
        s.group[e.ke].mon[e.id].spawn_exit = function () {
            if (s.group[e.ke].mon[e.id].isStarted === true) {
                ...
                s.orphanedVideoCheck(e, 2, null, true);
                s.onMonitorUnexpectedExitExtensions.forEach(function (
                    extender,
                ) {
                    extender(
                        Object.assign(s.group[e.ke].mon_conf[e.id], {}),
                        e,
                    );
                });
            }
        ...
        s.group[e.ke].mon[e.id].spawn.on('error', function (er) {
            s.userLog(e, {type: 'Spawn Error', msg: er});
            s.fatalCameraError(e, 'Spawn Error');
        });
        s.userLog(e, {
            type: lang['Process Started'],
            msg: {cmd: s.group[e.ke].mon[e.id].ffmpeg},
        });
        if (s.isWin === false) {
            s.group[e.ke].mon[e.id].getMonitorCpuUsage = setInterval(
                function () {
    ...
    s.createCameraStreamHandlers = function (e) {
        ...
        s.group[e.ke].mon[e.id].emitter =
            new events.EventEmitter().setMaxListeners(
                e.details.stream_mjpeg_clients,
            );
        if (e.type === 'jpeg') {
        ...
        if (e.details.detector_audio === '1') {
            ...
            var triggerLevelMax;
            if (
                e.details.detector_audio_min_db &&
                e.details.detector_audio_min_db !== ''
            ) {
            ...
            audioDetector.start();
            s.group[e.ke].mon[e.id].spawn.stdio[6].pipe(
                audioDetector.streamDecoder,
            );
        }
        if (e.details.detector === '1' && e.coProcessor === false) {
            ...
            if (e.details.detector_pam === '1') {
                ...
                s.group[e.ke].mon[e.id].spawn.stdio[3]
                    .pipe(s.group[e.ke].mon[e.id].p2p)
                    .pipe(s.group[e.ke].mon[e.id].pamDiff);
                if (e.details.detector_use_detect_object === '1') {
                    s.group[e.ke].mon[e.id].spawn.stdio[4].on(
                        'data',
                        function (d) {
        ...
        switch (e.details.stream_type) {
            case 'mp4':
                ...
                s.group[e.ke].mon[e.id].mp4frag['MAIN'].on(
                    'error',
                    function (error) {
                        s.userLog(e, {
                            type: lang['Mp4Frag'],
                ...
                s.group[e.ke].mon[e.id].spawn.stdio[1].pipe(
                    s.group[e.ke].mon[e.id].mp4frag['MAIN'],
                );
                break;
            case 'flv':
                e.frameToStream = function (d) {
                    if (!s.group[e.ke].mon[e.id].firstStreamChunk['MAIN'])
                        s.group[e.ke].mon[e.id].firstStreamChunk['MAIN'] = d;
                    e.frameToStream = function (d) {
                        s.resetStreamCheck(e);
        ...
        if (e.frameToStream) {
            if (
                e.coProcessor === true &&
                e.details.stream_type === ('b64' || 'mjpeg')
            ) {
            } else {
        ...
        if (e.details.stream_channels && e.details.stream_channels !== '') {
            var createStreamEmitter = function (channel, number) {
                ...
                if (!s.group[e.ke].mon[e.id].emitterChannel[pipeNumber]) {
                    s.group[e.ke].mon[e.id].emitterChannel[pipeNumber] =
                        new events.EventEmitter().setMaxListeners(0);
                }
                var frameToStream;
                switch (channel.stream_type) {
                    case 'mp4':
                        s.group[e.ke].mon[e.id].mp4frag[pipeNumber] =
                            new Mp4Frag();
                        s.group[e.ke].mon[e.id].spawn.stdio[pipeNumber].pipe(
                            s.group[e.ke].mon[e.id].mp4frag[pipeNumber],
                        );
                        break;
                    case 'mjpeg':
                ...
                if (frameToStream) {
                    s.group[e.ke].mon[e.id].spawn.stdio[pipeNumber].on(
                        'data',
                        frameToStream,
                    );
                }
        ...
        s.group[e.ke].mon[e.id].spawn.stderr.on('data', function (d) {
            ...
            switch (true) {
                ...
                case checkLog(d, 'error while decoding'):
                    s.userLog(e, {
                        type: lang['Error While Decoding'],
                        msg: lang.ErrorWhileDecodingText,
                    });
                    break;
                case checkLog(d, '[hls @'):
                ...
                case checkLog(d, 'Could not write header'):
                    return s.userLog(e, {
                        type: lang['Incorrect Settings Chosen'],
                        msg: {msg: d},
                    });
                    break;
                ...
                case checkLog(d, 'Connection timed out'):
                    ...
                    setTimeout(function () {
                        s.userLog(e, {
                            type: lang['Connection timed out'],
                            msg: lang['Retrying...'],
                        });
                    ...
                    s.insertCompletedVideo(
                        ...
                        function (err) {
                            ...
                            if (
                                ...
                                e.details.detector_delete_motionless_videos ===
                                    '1' &&
                                s.group[e.ke].mon[e.id]
                                ...
                                if (e.details.loglevel !== 'quiet') {
                                    s.userLog(e, {
                                        type: lang['Delete Motionless Video'],
                                        msg: filename,
                                    });
                                }
                                s.deleteVideo({
    ...
    s.launchMonitorProcesses = function (e) {
        ...
        var doOnThisMachine = function () {
            ...
            if (
                e.details.detector === '1' &&
                e.details.detector_notrigger === '1'
            ) {
                if (
                ...
                e.detector_notrigger_timeout =
                    parseFloat(e.details.detector_notrigger_timeout) *
                    1000 *
                    60;
                s.group[e.ke].mon[e.id].detector_notrigger_timeout_function =
                    function () {
                        s.onDetectorNoTriggerTimeoutExtensions.forEach(
                            function (extender) {
                                extender(e);
                            },
                        );
            ...
            if (
                ...
                (e.functionMode === 'record' ||
                    (e.functionMode === 'start' &&
                        e.details.detector_record_method === 'sip'))
            ) {
            ...
            if (s.group[e.ke].mon[e.id].isStarted === true) {
                ...
                startVideoProcessor = function (err, o) {
                    if (o.success === true) {
                        ...
                        s.onMonitorPingFailedExtensions.forEach(function (
                            extender,
                        ) {
                            extender(
                                Object.assign(s.group[e.ke].mon_conf[e.id], {}),
                        ...
                        s.userLog(e, {
                            type: lang['Ping Failed'],
                            msg: lang.skipPingText1,
                        });
                        s.fatalCameraError(e, 'Ping Failed');
                        return;
                    }
                };
                if (
        ...
        try {
            if (
                ...
                var copiedMonitorObject = s.cleanMonitorObject(
                    s.group[e.ke].mon_conf[e.id],
                );
                ...
                if (childNodeList.length > 0) {
                    ...
                    var selectNode = function (ip) {
                        ...
                        s.group[e.ke].mon[e.id].onChildNodeExit = function () {
                            if (s.childNodes[ip])
                                delete s.childNodes[ip].activeCameras[
                                    e.ke + e.id
                                ];
                        };
                    };
                    ...
                    childNodeList.forEach(function (ip) {
                        if (
                            Object.keys(s.childNodes[ip].activeCameras).length <
                            nodeWithLowestActiveCamerasCount
                        ) {
                            nodeWithLowestActiveCameras = ip;
                        }
                    });
                    if (nodeWithLowestActiveCameras)
                        selectNode(nodeWithLowestActiveCameras);
                    if (e.childNodeFound === true) {
                        s.childNodes[e.childNodeSelected].activeCameras[
                            e.ke + e.id
                        ] = copiedMonitorObject;
                        s.group[e.ke].mon[e.id].childNode = e.childNodeSelected;
                        s.group[e.ke].mon[e.id].childNodeId =
                            s.childNodes[e.childNodeSelected].cnid;
                        s.cx(
                            {
                                f: 'sync',
                                sync: s.group[e.ke].mon_conf[e.id],
                                ke: e.ke,
                                mid: e.id,
                            },
                            s.group[e.ke].mon[e.id].childNodeId,
    ...
    s.fatalCameraError = function (e, errorMessage) {
        ...
        if (s.group[e.ke].mon[e.id].isStarted === true) {
            s.group[e.ke].mon[e.id].err_fatal_timeout = setTimeout(function () {
                if (
                    e.details.fatal_max !== 0 &&
                    e.errorFatalCount > e.details.fatal_max
                ) {
    ...
    s.resetConnectionCamera = function (e) {
        var d = new Date();
        var n = d.toLocaleTimeString();
        setTimeout(function () {
            // s.sendMonitorStatus({id:e.id,ke:e.ke,status:lang.Starting});
            s.camera('start', e);
        }, e.details.fatal_max_timeOut * 1000 * 60);
    };
    s.isWatchCountable = function (d) {
        try {
            ...
            var indefiniteIgnore = ['mjpeg', 'h264'];
            var monConfig = s.group[d.ke].mon_conf[d.id];
            if (
                variableMethodsToAllow.indexOf(
                    monConfig.details.stream_type +
                        monConfig.details.stream_flv_type,
                ) > -1 &&
        ...
        s.sqlQuery(
            ...
            function (er, r) {
                ...
                if (r && r[0]) {
                    ...
                    Object.keys(form).forEach(function (v) {
                        if (form[v] && form[v] !== '') {
                            ...
                            if (v === 'details') {
                                if (form.camType === 'KDT') {
                                    //tdk://172.16.130.157:60002/mode=real&idc=1&ids=1
                                    desktopData.push(
                                        form.host,
                                        form.name,
                                        form.path,
                                        'tdk://' +
                                            form.host +
                                            ':60002/mode=real&idc=1&ids=1',
                                        form.mid + '.png',
                                        config.serverIp,
                                        form.ke,
                                        JSON.parse(form[v]).muser,
                                        JSON.parse(form[v]).mpass,
                                        form.camType,
                                    );
                                } else {
                                    desktopData.push(
                                ...
                                if (
                                    !JSON.parse(form[v]).max_keep_days ||
                                    JSON.parse(form[v]).max_keep_days === ''
                                ) {
                                    dataTbValueUpdate.push('30');
                                } else {
                                    dataTbValueUpdate.push(
                                        JSON.parse(form[v]).max_keep_days,
                                    );
                                }
                                if (
                                    !JSON.parse(form[v]).dir ||
                                    JSON.parse(form[v]).dir === 'undefined'
                                ) {
                                    dataTbValueUpdate.push('/storage/');
                                } else {
                                    dataTbValueUpdate.push(
                                        JSON.parse(form[v]).dir,
                                    );
                                }
                                dataTbValueUpdate.push(form.mid);
                                dataTbValueUpdate.push(form.ke);
                                desktopData.push(form.mid);
                            }
                    ...
                    endData.msg =
                        user.lang['Monitor Updated by user'] + ' : ' + user.uid;
                    s.sqlQuery(
                        'UPDATE Monitors SET ' +
                            monitorQuery +
                            ' WHERE ke=? AND mid=?',
                    ...
                    s.sqlQuery(
                        'UPDATE cleaningmonitors SET max_keep_day=? , path=? WHERE mid=? AND ke=?',
                        dataTbValueUpdate,
                    );
                    s.sqlQuery(
                        'UPDATE DesktopApp_camera SET ipaddr=?,name=?,path=?,fullurl=?,thumb=?,serverip=?,grpkey=?,username=?,password=?,type=? WHERE mid=? ',
                        desktopData,
                    );
                    affectMonitor = true;
                } else if (
                    ...
                    s.group[form.ke].init.max_camera === '' ||
                    Object.keys(s.group[form.ke].mon).length <=
                        parseInt(s.group[form.ke].init.max_camera)
                ) {
                    txData.new = true;
                    monitorQueryInsertValues = [];
                    //insert cleaningmonitors DATA
                    var dataTbValue = [];
                    Object.keys(form).forEach(function (v) {
                        if (form[v] && form[v] !== '') {
                            monitorQuery.push(v);
                            ...
                            if (v === 'details') {
                                ...
                                dataTbValue.push(form.mid);
                                dataTbValue.push(form.ke);
                                if (
                                    !JSON.parse(form[v]).max_keep_days ||
                                    JSON.parse(form[v]).max_keep_days === ''
                                ) {
                                ...
                                if (
                                    ...
                                    dataTbValue.push('/storage/');
                                } else {
                                    dataTbValue.push(JSON.parse(form[v]).dir);
                                }
                            }
                            monitorQueryValues.push(form[v]);
                        }
                    ...
                    monitorQuery = monitorQuery.join(',');
                    monitorQueryInsertValues =
                        monitorQueryInsertValues.join(',');
                    s.userLog(form, {
                        type: 'Monitor Added',
                        msg: 'by user : ' + user.uid,
                    });
                    endData.msg =
                        user.lang['Monitor Added by user'] + ' : ' + user.uid;
                    s.sqlQuery(
                    ...
                    s.sqlQuery(
                        'INSERT INTO cleaningmonitors (mid,ke,max_keep_day,path) VALUES (?,?,?,?)',
                        dataTbValue,
                    );
                    s.sqlQuery(
                        'INSERT INTO DesktopApp_camera (ipaddr,name,path,fullurl,thumb,serverip,grpkey,mid,username,password,type) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
                        desktopData,
                    );
                    affectMonitor = true;
                } else {
                ...
                if (affectMonitor === true) {
                    ...
                    s.group[form.ke].mon_conf[form.mid] =
                        s.cleanMonitorObject(form);
                    if (form.mode === 'stop') {
                        s.camera('stop', form);
                    } else {
                        s.camera('stop', form);
                ...
                s.onMonitorSaveExtensions.forEach(function (extender) {
                    extender(
                        Object.assign(s.group[form.ke].mon_conf[form.mid], {}),
                        form,
                        endData,
                    );
                });
    ...
    s.camera = function (x, e, cn) {
        ...
        switch (e.functionMode) {
            case 'watch_on': //live streamers - join
                ...
                if (!cn.monitorsCurrentlyWatching[e.id]) {
                    cn.monitorsCurrentlyWatching[e.id] = {ke: e.ke};
                }
                s.group[e.ke].mon[e.id].watch[cn.id] = {};
                var numberOfViewers = Object.keys(
            ...
            case 'watch_off': //live streamers - leave
                if (cn.monitorsCurrentlyWatching) {
                    delete cn.monitorsCurrentlyWatching[e.id];
                }
                var numberOfViewers = 0;
                delete s.group[e.ke].mon[e.id].watch[cn.id];
                numberOfViewers = Object.keys(
                    s.group[e.ke].mon[e.id].watch,
                ).length;
                s.tx(
                    {
            ...
            case 'stop': //stop monitor
                ...
                if (
                    config.childNodes.enabled === true &&
                    config.childNodes.mode === 'master' &&
                    s.group[e.ke].mon[e.id].childNode &&
                    s.childNodes[s.group[e.ke].mon[e.id].childNode]
                        .activeCameras[e.ke + e.id]
                ) {
                    ...
                    if (e.functionMode === 'stop') {
                        ...
                        if (e.delete === 1) {
                            s.group[e.ke].mon[e.id].delete = setTimeout(
                                function () {
            ...
            case 'record': //watch or record monitor url
                ...
                if (!s.group[e.ke].mon_conf[e.id]) {
                    s.group[e.ke].mon_conf[e.id] = s.cleanMonitorObject(e);
                }
                ...
                //lock this function
                s.sendMonitorStatus({id: e.id, ke: e.ke, status: e.mode});
                s.group[e.ke].mon[e.id].isStarted = true;
                ...
                if (e.details.fatal_max_timeOut === '') {
                    e.details.fatal_max_timeOut = 5;
                } else {
                    e.details.fatal_max_timeOut = parseFloat(
                        e.details.fatal_max_timeOut,
                    );
                }
                e.errorFatalCount = 0;
    ...
    s.activateMonitorStates = function (groupKey, stateName, user, callback) {
        ...
        s.findPreset(
            [groupKey, 'monitorStates', stateName],
            function (notFound, preset) {
                if (notFound === false) {
                        ...
                        function (err, monitors) {
                            if (monitors && monitors[0]) {
                                monitors.forEach(function (monitor) {
                                    ...
                                    var monitorPreset =
                                        monitorPresets[monitor.mid];
                                    monitorPreset.details = Object.assign(
                                        monitor.details,
                                        monitorPreset.details,
                                    );
                                    monitor = s.cleanMonitorObjectForDatabase(
                                        Object.assign(monitor, monitorPreset),
                                    );
                                    monitor.details = JSON.stringify(
                                        monitor.details,
                                    );
                                    s.addOrEditMonitor(
                                ...
                                endData.msg =
                                    user.lang[
                                        'State Configuration has no monitors associated'
                                    ];
                                callback(endData);
                            }
                        },
            ...
            Object.keys(s.group[monitor.ke].mon_conf).forEach(function (v) {
                try {
                    var groups = JSON.parse(
                        s.group[monitor.ke].mon_conf[v].details.groups,
                    );
                    if (!groups) {
                        groups = [];
                    }
                ...
                if (!list[v] && groups.indexOf(b) > -1) {
                    ...
                    if (s.group[monitor.ke].mon_conf[v].mode !== 'stop') {
                        cameras.push(
                            Object.assign({}, s.group[monitor.ke].mon_conf[v]),
                        );
                    }
                }
            });
