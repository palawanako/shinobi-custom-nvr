...
module.exports = function(s,config,lang,app,io){
    ...
    * API : add screen shot
    */
    app.post('/ScreenShotPost', function(req, res) {
        var data_url = req.body.imgBase64;
        var m_id = req.body.m_id;
        var data = data_url.replace(/^data:image\/\w+;base64,/, "");
        var buffer = Buffer.from(data, 'base64');

        fs.writeFile('web/libs/snap/'+m_id+'.png', buffer, function (err) {
            if(err){
                console.log('error snap shot');
                console.log(err);
            }
            console.log('done snap shot');
            res.send('success');
        })
    });
    /**
                        ...
                        if(r.details.sub){
                            s.sqlQuery('SELECT details FROM Users WHERE ke=? AND details NOT LIKE ?',[r.ke,'%"sub"%'],function(err,rr) {
                                if(rr && rr[0]){
                                    ...
                                    r.details.depart_groups=rr.details.depart_groups;
        ...
        req.fn=function(user){
            ...
            if(!req.params.id){
                if(user.details.sub&&user.details.monitors&&user.details.allmonitors!=='1'){
                    try{user.details.monitors=JSON.parse(user.details.monitors);}catch(er){}
                    req.or=[];
                    if(user.details.monitors.length >0){
                        user.details.monitors.forEach(function(v,n){
                            req.or.push('mid=?');
                            req.ar.push(v)
                        })
    ...
    */
    //TAMEYE API LIST
    app.get([
        config.webPaths.apiPrefix+':auth/getAllCamerasStart/:ke',
    ], function (req,res){
        s.auth(req.params,function(user){
            var endData = {
                ok : false
            ...
            s.sqlQuery("SELECT mid,mode FROM Monitors WHERE mode=?",['start'],function(err,cameras){
                if(cameras && cameras[0]){
                    endData.ok = true
                    /*cameras.forEach(function(camera){
                        camera.details = JSON.parse(camera.details)
                    })*/
                    endData.cameras = cameras
                }else{
    ...
    //return events for motion
    app.get([
        config.webPaths.apiPrefix+':auth/tamevents/:ke',
    ], function (req,res){
        s.auth(req.params,function(user){
            var endData = {
                ok : false
            ...
            s.sqlQuery("SELECT * FROM Events",function(err,evensts){
            //s.sqlQuery("SELECT mid,mode FROM Monitors WHERE mode=?",['start'],function(err,cameras){
                if(evensts && evensts[0]){
                    endData.ok = true
                    evensts.forEach(function(evenst){
                        evenst.details = JSON.parse(evenst.details)
                    })
                    endData.evensts = evensts
                }else{
                    endData.msg = user.lang['No resault found']
                }
                s.closeJsonResponse(res,endData)
    ...
    //return events for motion by cam ID
    app.get([
        config.webPaths.apiPrefix+':auth/tamevent/:ke/:id/:start/:end',
    ], function (req,res){
        s.auth(req.params,function(user){
            var endData = {
                ok : false
            ...
            if(req.params.start&&req.params.start!=='') {
                let tmpstart = req.params.start.split('T')
                let year = tmpstart[0]
                let time = tmpstart[1].replace(/-/g, ':')
                req.start = year + ' ' + time
            }
            if(req.params.end&&req.params.end!==''){
                let tmpend = req.params.end.split('T')
                let year = tmpend[0]
                let time = tmpend[1].replace(/-/g, ':')
                req.end = year + ' ' + time
            }
           s.sqlQuery("SELECT * FROM Events WHERE mid=? AND time >= ? AND time <= ?",[req.params.id,req.start,req.end],function(err,events){
                if(events && events[0]){
                    endData.ok = true
                    events.forEach(function(event){
                        event.details = JSON.parse(event.details)
                        event.time=s.utcToLocal(event.time)
                    })
                    endData.evensts = events
                }else{
                    endData.msg = user.lang['No resault found']
                }
                s.closeJsonResponse(res,endData)
    ...
    app.get([
        config.webPaths.apiPrefix+':auth/tamcountcams/:ke',
    ], function (req,res){
        s.auth(req.params,function(user){
            var endData = {
                ok : false
            ...
            s.sqlQuery("SELECT COUNT(*) as count FROM Monitors",function(err,data){
                if(data){
                   // endData.ok = true
                    endData = data
                }else{
    ...
    //return Sum Users
    app.get([
        config.webPaths.apiPrefix+':auth/tamcountUsers/:ke',
    ], function (req,res){
        s.auth(req.params,function(user){
            var endData = {
                ok : false
            ...
            s.sqlQuery("SELECT COUNT(*) as sumUsers FROM Users WHERE mail IS NOT NULL",function(err,data){
                if(data){
                    // endData.ok = true
                    endData = data
                }else{
    ...
    //return count mode camera
    app.get([
        config.webPaths.apiPrefix+':auth/tamcountmode/:ke/:mode',
    ], function (req,res){
        s.auth(req.params,function(user){
            var endData = {
               // ok : false
            ...
            s.sqlQuery("SELECT COUNT(*) as count FROM Monitors WHERE mode=?",[req.params.mode],function(err,data){
                if(data){
                   // endData.ok = true
                    endData = data
                }else{
    ...
    //return harDisk Usage
    app.get([
        config.webPaths.apiPrefix+':auth/tamCheckDisk/:ke',
    ], function (req,res){
            ...
            s.sqlQuery('SELECT * FROM Videos WHERE ke=? AND status!=?',[user.ke,0],function(err,videos){
                if(videos && videos[0]){
                    videos.forEach(function(video){
                        user.DiskUsed += video.size
                    })
                }
                endData.totalDisk = user.details.size;
                endData.diskUsed = user.DiskUsed
               s.closeJsonResponse(res,endData)
     ...
     /**
         * API : convert Video File
         */
        app.get(config.webPaths.apiPrefix+':auth/videosts/:ke/:id/:file', function (req,res){
            s.auth(req.params,function(user){
                if(user.permissions.watch_videos==="0"||user.details.sub&&user.details.allmonitors!=='1'&&user.details.monitors.indexOf(req.params.id)===-1){
                    res.end(user.lang['Not Permitted'])
                    return
                ...
                s.sqlQuery('SELECT * FROM Videos WHERE ke=? AND mid=? AND `time`=? LIMIT 1',[req.params.ke,req.params.id,time],function(err,r){
                    if(r&&r[0]){
                        ...
                        if (fs.existsSync(req.dir)){
                            ...
                            //get directory of video
                            var directoryVideo=s.getVideoDirectory(r[0])
                            //compelete address mp4 file
                            var completeAddmp4=req.dir;
                            //get name of video
                            var tmpFname = req.params.file.split('.');
                            var fName=tmpFname[0]

                            //compelete TS file
                            var tsfilepath=directoryVideo+''+ fName + '.ts';

                            if (!fs.existsSync(tsfilepath)){
                                var convertTots = spawn(config.ffmpegDir,s.splitForFFPMEG(('-i ' + completeAddmp4 + ' -c copy  ' + directoryVideo +''+ fName + '.ts')))
                                convertTots.stdout.on('data', function(data) {
                                    console.log('stdout----');
                                });
                                convertTots.stderr.on('data', function(data) {
                                    console.log('stderr---');
                                });
                                convertTots.on('close', function() {
                                    s.streamTsFileOverHttp(tsfilepath,req,res)
                                });
                            }else{
                                s.streamTsFileOverHttp(tsfilepath,req,res)
                            }
         ...
         */
        app.get([
            config.webPaths.apiPrefix+':auth/videosts/:ke/:id/:file/:mode',
            config.webPaths.apiPrefix+':auth/videosts/:ke/:id/:file/:mode/:f',
        ], function (req,res){
            req.ret={ok:false};
            res.setHeader('Content-Type', 'application/json');
            s.auth(req.params,function(user){
                ...
                s.sqlQuery(req.sql,req.ar,function(err,r){
                    if(r&&r[0]){
                        r=r[0];
                        r.filename=s.formattedTime(r.time)+'.ts';
                        switch(req.params.mode){
                            case'fix':
                                req.ret.ok=true;
                                s.video('fix',r)
