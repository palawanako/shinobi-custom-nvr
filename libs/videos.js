            ...
            if(r && r[0]){
                ...
                fs.chmod(e.dir+filename,0o777,function(err){
                    ...
                    fs.unlink(e.dir+filename,function(err){
                        console.log('debug1_ERROR_unlink')
...
//                    console.error(err)
    ...
    s.streamTsFileOverHttp = function(filePath,req,res){
        var ext = filePath.split('.')
        ext = filePath[filePath.length - 1]
        var total = fs.statSync(filePath).size;
        if (req.headers['range']) {
            ...
            req.headerWrite={ 'Content-Length': total, 'Content-Type': 'video/ts'};
            var file = fs.createReadStream(filePath)
            req.writeCode=200
        }
        if(req.query.downloadName){
