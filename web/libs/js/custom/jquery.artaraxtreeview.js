...
(function ($) {
    ...
    $.artaraxTreeView = function (options) {
        ...
        var defaults = {
            ...
            mode: "deletable,autoSelectChildren",
            updateCallBack: null,
            deleteCallBack: null
        };
        ...
        function loadTreeView(parentId, isOnUpdate) {
            ...
            $.each(childItems, function (index, item) {
                ...
                var hasChildrenFlag = hasChildren(item.iddep);
                if (hasChildrenFlag) {
                    treeViewEndTag = '</ul></li>';
                }

                appendTreeItem(item, hasChildrenFlag, getCheckedAttribute(item.iddep, isOnUpdate));

                // returning method
                loadTreeView(item.iddep, isOnUpdate);
        ...
        function appendTreeItem(obj, hasChildren, checkedAttribute) {
            ...
            if (isUpdatable) {
                updatableHtml = '<img class="mng-img node-update" data-id="' + obj.iddep + '" src="libs/img/update.png"/>';
            }

            if (isDeletable) {
                deletableHtml = '<img class="mng-img node-delete" data-id="' + obj.iddep + '" src="libs/img/delete.png"/>';
            }
            ...
            if (hasChildren) {
                treeViewHtml += '<li><span class="' + nodeSignCssClass + '"></span><label class="' + selectedClass + '"><input type="checkbox" data-id="' + obj.iddep + '" ' + checkedAttribute + ' class="chk" /> ' + obj.namedep + '</label><span class="btn-wrpr">' + updatableHtml + deletableHtml + '</span>';
                treeViewHtml += '<ul>';
            }
            else {
                treeViewHtml += '<li><span class="node-none"></span><label class="' + selectedClass + '"><input type="checkbox" data-id="' + obj.iddep + '" ' + checkedAttribute + ' class="chk leaf" /> ' + obj.namedep + '</label><span class="btn-wrpr">' + updatableHtml + deletableHtml + '</span></li>';
            }
        }
