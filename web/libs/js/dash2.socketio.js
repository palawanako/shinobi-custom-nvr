...
$.ccio.globalWebsocket=function(d,user){
    ...
    switch(d.f){
        ...
        case'monitor_status':
            ...
            $('[ke="'+d.ke+'"][mid="'+d.id+'"][auth="'+user.auth_token+'"] .tamLamp').attr('title',d.status)
        break;
        ...
        case'os'://indicator
            //RAM
            d.myram=(parseFloat(d.ram)).toFixed(0)+'%';
            $('#ramUsage .bar').css('width',d.myram);
            $('#ramUsage .bar-percentage').html(d.myram)
            //remove loader
            $(".v-align").fadeOut("slow");
            //cpu
            d.cpu=parseFloat(d.cpu).toFixed(0)+'%';
            $('#cpuUsage .bar').css('width',d.cpu);
            $('#cpuUsage .bar-percentage').html(d.cpu)

            //disk
            if(!user.details.size||user.details.size===''){
                if(!d.totalDisk||d.totalDisk===''){
                    user.details.size=10000
                }else{
                    user.details.size=d.totalDisk
                }
            }
            d.percent=parseInt(((d.disk/1024)/user.details.size)*100)+'%';
            d.human=parseFloat((d.disk/1024))
            if(d.human>1000){d.human=(d.human/1000).toFixed(2)+' GB'}else{d.human=d.human.toFixed(2)+' MB'}
            $('#diskUsage .value').html(d.human)
            $('#diskUsage .bar').css('width',d.percent)
            $('#diskUsage .bar-percentage').html(d.percent)
...
//                                    $.ccio.mon[d.ke+d.id+user.auth_token].image.src = base64Frame
                    ...
                    case'mp4':
                        setTimeout(function(){
                            ...
                            var ccio =$.ccio.mon[d.ke+d.id+user.auth_token]
                            var onPoseidonError = function(e){
                                // setTimeout(function(){
                            ...
                            if(d.d.stream_flv_type==='ws'){
                                ...
                                try{
                                    var vidElement=$('#monitor_live_'+d.id+user.auth_token)
                                    var btnResize=$('#monitor_live_'+d.id+user.auth_token).children('.ui-resizable-handle')
                                    let curPlayerW
                                    let curPlayerH
                                    var tempObj={}
                                    var swArray
                                    var isSwitch=false;
                                    var userSetSwitch={}
                                    var monDD=JSON.parse($.ccio.mon[d.ke+d.id+user.auth_token].details)
                                    tempObj=JSON.stringify(monDD)
                                    var monDDTemp=JSON.parse(tempObj)

                                    //for btn switcher
                                    var vidM=$('video#'+d.id)
                                    var btnChangeStream= $(vidM).parent('.stream-block').parent('.grid-stack-item-content').siblings('.mdl-card__supporting-text').children('.btn-group-sm').children('.btn-changStream')
                                    $(btnChangeStream).unbind('click')
                                    $(btnChangeStream).on('click',function (e) {
                                        var btnSwitch
                                        try{
                                            btnSwitch = JSON.parse(monDDTemp.btn_switch_stream)
                                        }catch(er){
                                            btnSwitch = monDDTemp.btn_switch_stream;
                                        }
                                        if(monDDTemp.btnSwitch &&  monDDTemp.btnSwitch==='1') {
                                            isSwitch=true
                                            changeStream(btnSwitch[0].streamchoices,monDD,isSwitch)
                                        }
                                    })


                                    //for resizer
                                    if(monDDTemp.switch_stream){
                                        swArray=JSON.parse(monDDTemp.switch_stream)
                                        if(swArray[0].changStream1_X){
                                            userSetSwitch.width_1=parseInt(swArray[0].changStream1_X)
                                            userSetSwitch.hight_1=parseInt(swArray[0].changStream1_Y)
                                            userSetSwitch.stream_1=swArray[0].changStream1_SS
                                        }
                                        if(swArray[0].changStream2_X){
                                            userSetSwitch.width_2=parseInt(swArray[0].changStream2_X)
                                            userSetSwitch.hight_2=parseInt(swArray[0].changStream2_Y)
                                            userSetSwitch.stream_2=swArray[0].changStream2_SS
                                        }
                                    }

                                    $(btnResize).on( "click", function(event) {
                                        var camDetails=(JSON.parse($.ccio.mon[d.ke+d.id+user.auth_token].details))
                                        event.stopImmediatePropagation()
                                        //check change stream YES
                                        if(camDetails.changStream==='1'){
                                            curPlayerW=vidElement[0].clientWidth
                                            curPlayerH=vidElement[0].clientHeight
                                            if(userSetSwitch.width_1 && (curPlayerW<=userSetSwitch.width_1 && curPlayerH<=userSetSwitch.hight_1) ){
                                                isSwitch=false;
                                                changeStream(userSetSwitch.stream_1,camDetails,isSwitch)
                                            }
                                            else if(userSetSwitch.width_2 &&
                                                (curPlayerW<=userSetSwitch.width_2 && curPlayerH<=userSetSwitch.hight_2)
                                                &&
                                                (curPlayerW>=userSetSwitch.width_1 && curPlayerH>=userSetSwitch.hight_1)
                                            )
                                            {
                                                isSwitch=false;
                                                changeStream(userSetSwitch.stream_2,camDetails,isSwitch)
                                            }else{
                                                isSwitch=true;
                                                var mainStr='1'
                                                changeStream(mainStr,camDetails,isSwitch)
                                            }
                                        }
                                    });
                            ...
                            $.get(d.url,function(m3u){
                                if(m3u=='File Not Found'){
                                    ...
                                    $(btnResize).click(function(event) {
                                        var camDetails_hls=(JSON.parse($.ccio.mon[d.ke+d.id+user.auth_token].details))
                                        event.stopImmediatePropagation()
                                        //changStream
                                        //check change stream YES
                                        if(camDetails_hls.changStream==='1'){
                                            curPlayerW=vidElement[0].clientWidth
                                            curPlayerH=vidElement[0].clientHeight
                                            if(userSetSwitch.width_1 && (curPlayerW<=userSetSwitch.width_1 && curPlayerH<=userSetSwitch.hight_1) ){
                                                isSwitch=false;
                                                changeStream(userSetSwitch.stream_1,camDetails_hls,isSwitch)
                                            }
                                            else if(userSetSwitch.width_2 &&
                                                (curPlayerW<=userSetSwitch.width_2 && curPlayerH<=userSetSwitch.hight_2)
                                                &&
                                            ...
                                            {
                                                isSwitch=false;
                                                changeStream(userSetSwitch.stream_2,camDetails_hls,isSwitch)
                                            }else{
                                                isSwitch=true;
                                                var mainStr='1'
                                                changeStream(mainStr,camDetails_hls,isSwitch)
                                            }
                                        }
                                    });
...
//                        ctx.getContext("2d").drawImage(image,d.x,d.y,d.width,d.height)
        ...
        case'monitor_edit':
            ...
            //for show icon change stream in icons list
            if(d.mon.details.btnSwitch=="1"){
                d.e.find('.btn-changStream').show()
            }else{
                d.e.find('.pad').remove();
                d.e.find('.btn-changStream').hide()
            }
            if(user===$user){
                d.chosen_set='watch_on'
            }else{
            ...
            if(!d.silenceNote){
               // $.ccio.init('note',{title:'Monitor Saved',text:'<b>'+d.mon.name+'</b> <small>'+d.mon.mid+'</small> has been saved.',type:'success'});
                $.ccio.init('note',{title:lang['Monitor Saved'],text:'<b>'+d.mon.name+'</b> <small>'+d.mon.mid+'</small> '+lang['has been saved'],type:'success'});
            }
        break;
...
//                v.filename=$.ccio.init('tf',v.time)+'.'+v.ext;
    ...
    var selectedInp=lang['inpMap_main'];
    var tempObj={}


    var changeStream=function (streamValue,monDD,isSwitch) {
        //object for compare and if any changes exist then send to API
        tempObj=JSON.stringify(monDD)
        var monDDTemp=JSON.parse(tempObj)

        //check input map exist
        var tmpInputStream=[]
        if(typeof monDDTemp.input_maps==='string' ){
            if(monDDTemp.input_maps.length>2){
                tmpInputStream=JSON.parse(monDDTemp.input_maps)
            }else{
                tmpInputStream=[]
            }
        }else{
            tmpInputStream=monDDTemp.input_maps
        }
        if(tmpInputStream.length >0 ){
            $.map( monDD, function( value, key ) {
                if(key==='input_map_choices'){
                    if(value){
                        if(typeof monDD[key]==='string' ){
                            monDD[key]=JSON.parse(monDD[key])
                        }else{
                            monDD[key]=monDD[key]
                        }
                    }else{
                        //not set input map so init default values
                        monDD[key]={"stream":[],"snap":[],"record":[],"detector":[],"detector_sip_buffer":[]}
                    }

                    if(isSwitch){
                        $.each(monDD[key], function(k, v) {
                            if(k==='stream'){
                                if(v[0] && (v[0].map==='1' || v[0].map==='2')){
                                    monDD[key][k]=[]
                                    selectedInp=lang['inpMap_main'];
                                }else if(v || v[0].length===0){
                                    monDD[key][k]=[{"map":streamValue}]
                                    selectedInp=lang['inpMap_second'];
                                }
                            }
                        });
                    }else{
            ...
            //method call API to update
            updateCamFields(monDD,monDDTemp)
        }else{
            //if not set input map show notification
            new PNotify({
                title : 'camera :'+d.id,
                text :  lang['check_input_map_notSet'],
                type : 'info'
            })
        }
    }
    var updateCamFields=function (monDD,monDDTemp) {
        var cameraFields={}
        //prepare Object
        cameraFields.details=monDD
        cameraFields.ext=$.ccio.mon[d.ke+d.id+user.auth_token].ext
        cameraFields.fps=$.ccio.mon[d.ke+d.id+user.auth_token].fps
        cameraFields.height=$.ccio.mon[d.ke+d.id+user.auth_token].height
        cameraFields.host=$.ccio.mon[d.ke+d.id+user.auth_token].host
        cameraFields.mid=$.ccio.mon[d.ke+d.id+user.auth_token].mid
        cameraFields.mode=$.ccio.mon[d.ke+d.id+user.auth_token].mode
        cameraFields.name=$.ccio.mon[d.ke+d.id+user.auth_token].name
        cameraFields.path=$.ccio.mon[d.ke+d.id+user.auth_token].path
        cameraFields.port=$.ccio.mon[d.ke+d.id+user.auth_token].port
        cameraFields.protocol=$.ccio.mon[d.ke+d.id+user.auth_token].protocol
        cameraFields.shfr=$.ccio.mon[d.ke+d.id+user.auth_token].shfr
        cameraFields.shto=$.ccio.mon[d.ke+d.id+user.auth_token].shto
        cameraFields.type=$.ccio.mon[d.ke+d.id+user.auth_token].type
        cameraFields.width=$.ccio.mon[d.ke+d.id+user.auth_token].width

        if(!checkObj(monDD,monDDTemp)){
            $.post($.ccio.init('location',user)+user.auth_token+'/configureMonitor/'+user.ke+'/'+d.id,{data:JSON.stringify(cameraFields)},function(data){
                //get tag for display current stream
                var curStream_string= $("#monitor_live_"+d.id+user.auth_token).find("#inpStream")
                curStream_string.text(selectedInp)
                setTimeout(function(){
                    $.ccio.cx({f:'monitor',ff:'watch_on',id:d.id},user)
