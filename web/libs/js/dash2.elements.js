$(document).ready(function(e){
    ...
    //Group Selector
    $.gR2={e:$('#group_list'),b:$('#group_list_button')};
    $.gR2.drawList=function(){
        var e={};
        e.tmp='';
    ...
    $.gR2.e.on('click','[groups]',function(){
        var e={};
        e.e=$(this),
            e.a=e.e.attr('groups');
    ...
    $.gR.drawList=function(){
        var e={};
        e.tmp='';
        e.tmp='<div class="btn-group btn-block dropup" role="group">\n' +
            '    <button id="btnGroupDrop1" type="button" class="btn btn-block btn-success dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'+lang['Select Department']+'</button>' +
            '    <div class="dropdown-menu btn-block" aria-labelledby="btnGroupDrop1">';
        e.tmp+='<div id="treeFilter"></div>'
        e.tmp+='</div></div>'
        $.gR.e.html(e.tmp)

        var dataArrayTreeFilter = [];
        for(var o in $user.depart_groups) {
            dataArrayTreeFilter.push($user.depart_groups[o]);
        }
        var treeFilter = simTree({
            el: '#treeFilter',
            data: dataArrayTreeFilter,
            check: true,
            linkParent: true,
            ...
            onChange: function (item) {
                var classes = [];
                for(var myk in item){
                    classes.push(item[myk]['iddep']);
                }
                filteredDep=classes;
                $('.filterDiv').each(function(k,v) {
                    var that = $(this);
                    var show = false;
        ...
        //treeFilter.expandNode('001')
        treeFilter.setSelected(filteredDep)
    }
    ...
    //to change view from list to graphical
    $('#ViewCamGraphical').click(function(){
        $('#left_menu').removeClass('list-blocks')
        $('.monitor_block').removeClass('col-md-5')
        $('.monitor_block').addClass('display-flx')
        $('#left_menu').addClass('shortView')
    })
    $('#viewGrid').click(function(){
        $('#left_menu').removeClass('shortView')
        $('#left_menu').addClass('list-blocks')
        $('.monitor_block').addClass('col-md-5')
        $('.monitor_block').removeClass('display-flx')
    })
    ...
    .on('click','[video]',function(e){
        ...
        switch(e.a){
            case'launch':
                ...
                //var html = '<video id="popPlayer" video="'+e.href+'" autoplay loop class="" controls data-setup="{}"><source src="'+e.href+'" type="video/mp4"></video><br><small class="msg"></small>'
                // var html = '<video id="vidSm"  class="" controls preload="none" width="300" height="264"  data-setup="{}" ><source src="'+e.href+'" type="video/mp4"></video><br><small class="msg"></small>'
                 // $.vidview.preview.html('<video  id="vidSm" class="" controls preload="none" width="300" height="264"  data-setup="{}" ><source src="'+e.attr('href')+'" type="video/mp4"></video>')

                modalBody.html('<video id="popPlayer"  class="video_video" controls preload="none" width="300" height="264"  data-setup="{}" ><source src="'+e.href+'" type="video/mp4"></video><br><small class="msg"></small>')
                const playerpop = new Plyr('#popPlayer');
    ...
    .on('click','[monitor]',function(){
      var e={};
        ...
        switch(e.a){
            ...
            break;
            case'multivideo':
                    $.timelapse1.e.modal('show')
                    $.timelapse1.monitors1.find('.monitor').remove()
                    $.timelapse2.monitors2.find('.monitor').remove()
                    $.timelapse3.monitors3.find('.monitor').remove()
                    $.timelapse4.monitors4.find('.monitor').remove()
                    $.each($.ccio.mon,function(n,v){
                        $.timelapse1.monitors1.append('<option class="monitor" value="'+v.mid+'">'+v.name+'</option>')
                        $.timelapse2.monitors2.append('<option class="monitor" value="'+v.mid+'">'+v.name+'</option>')
                        $.timelapse3.monitors3.append('<option class="monitor" value="'+v.mid+'">'+v.name+'</option>')
                        $.timelapse4.monitors4.append('<option class="monitor" value="'+v.mid+'">'+v.name+'</option>')
                    })
                    e.e=$.timelapse1.monitors1.find('.monitor').prop('selected',false)
                    e.e=$.timelapse2.monitors2.find('.monitor').prop('selected',false)
                    e.e=$.timelapse3.monitors3.find('.monitor').prop('selected',false)
                   e.e=$.timelapse4.monitors4.find('.monitor').prop('selected',false)
                    if(e.mid!==''){
                        e.e=$.timelapse1.monitors1.find('.monitor[value="'+e.mid+'"]')
                        e.e=$.timelapse2.monitors2.find('.monitor[value="'+e.mid+'"]')
                        e.e=$.timelapse3.monitors3.find('.monitor[value="'+e.mid+'"]')
                        e.e=$.timelapse4.monitors4.find('.monitor[value="'+e.mid+'"]')
                    }
                    e.e.first().prop('selected',true)
                    //$.timelapse1.f.submit()
            break;
                ...
                $.getJSON(e.videoURL,function(d){
                    ...
                    switch(e.a){
                        ...
                        case'videos_table':
                            ...
                            var tmp = '<table class="table table-striped listVideo" style="max-height:500px">';
                            tmp+='<thead>';
                            tmp+='<tr>';
    ...
    //                                v.filename=$.ccio.init('tf',v.time)+'.'+v.ext;
                                    ...
                                    tmp+='<td title="'+v.end+'">'+$.ccio.timeObject(v.end).locale('fa').format('jYYYY/jMM/jDD ,h:mm:ss A')+'</td>';
                                    tmp+='<td title="'+v.time+'">'+$.ccio.timeObject(v.time).locale('fa').format('jYYYY/jMM/jDD ,h:mm:ss A')+'</td>';
            ...
            case'zoom':
                  var  videoTag2 = $('#'+e.mid+'')
                   var videoTag = videoTag2[0]
                    ...
                    e.e=e.p.find('.Zoom_controls');
                    if(e.e.length>0){
                        e.e.remove()
                    }else{
                        var html = '<div class="Zoom_controls">'
                        html += '<div class="pad">'
                        html += '<div class="control top" id="myUp"  control="up"></div>'
                        html += '<div class="control left" id="myLeft" control="left"></div>'
                        html += '<div class="control right" id="myRight" control="right"></div>'
                        html += '<div class="control bottom" id="myDown" control="down"></div>'
                        html += '<div class="control middle" id="myCenter" control="center"></div>'
                        html += '</div>'
                        html += '<div class="btn-group btn-group-sm btn-group-justified">'
                        html += '<a title="zoom In" class="zoom_in btn btn-zoom" id="myZoomIn"  control="zoom_in"><i class="fa fa-search-plus"></i></a>'
                        html += '<a title="zoom Out" class="zoom_out btn btn-zoom" id="myZoomOut"  control="zoom_out"><i class="fa fa-search-minus"></i></a>'
                        html += '</div>'
                        html += '<div class="btn-group btn-group-sm btn-group-justified">'
                      html += '</div>'
                        html += '</div>'
                    ...
                    var intervalUp
                    var intervalDown
                    e.p.on('click','.Zoom_controls #myZoomIn',function(){
                        zoom = zoom + 0.9;
                        videoTag.style['transform']='scale('+zoom+')';
                    });
                    e.p.on('click','.Zoom_controls #myZoomOut',function(){
                        zoom = zoom - 0.9;
                        videoTag.style['transform']='scale('+zoom+')';
                    });
                    //LEFT
                    e.p.on('mouseover','.Zoom_controls #myLeft',function(){
                        intervalLeft =  setInterval(function() {
                            videoTag.style.left = (parseInt(videoTag.style.left,10) + 20) + 'px';
                        }, 100);
                    });
                    e.p.on('mouseout','.Zoom_controls #myLeft',function(){
                        console.log(videoTag.style.left)
                        clearInterval(intervalLeft);
                    });
                    //RIGHT
                    e.p.on('mouseover','.Zoom_controls #myRight',function(){
                       // videoTag.style['transform']='0.5s ease-out';
                        intervalRight =  setInterval(function() {
                            videoTag.style.left = (parseInt(videoTag.style.left,10) - 20) + 'px';
                        }, 100);
                    });
                    e.p.on('mouseout','.Zoom_controls #myRight',function(){
                        clearInterval(intervalRight);
                    });
                    //UP
                    e.p.on('mouseover','.Zoom_controls #myUp',function(){
                        intervalUp =  setInterval(function() {
                            videoTag.style.top = (parseInt(videoTag.style.top,10) + 10) + 'px';
                        }, 100);
                    });
                    e.p.on('mouseout','.Zoom_controls #myUp',function(){
                        clearInterval(intervalUp);
                    });
                    //DOWN
                    e.p.on('mouseover','.Zoom_controls #myDown',function(){
                        intervalDown =  setInterval(function() {
                            videoTag.style.top = (parseInt(videoTag.style.top,10) - 10) + 'px';
                        }, 100);
                    });
                    e.p.on('mouseout','.Zoom_controls #myDown',function(){
                        clearInterval(intervalDown);
                    });
                    e.p.on('click','.Zoom_controls #myCenter',function(){
                        zoom = 1;
                        videoTag.style.top = 0 + 'px';
                        videoTag.style.left = 0 + 'px';
                        videoTag.style['transform']='scale('+zoom+')';
                    });
