...
$.aM.import=function(e){
    ...
    $.aM.e.find('.ipCam').text(e.values.host);
    $.aM.e.find('.nameCam').text(e.values.name);
    ...
    //get SWITCH
    if(e.ss.switch_stream && e.ss.switch_stream!==''){
        var stream_channelsA
        try{
            stream_channelsA = JSON.parse(e.ss.switch_stream)
        }catch(er){
            stream_channelsA = e.ss.switch_stream;
        }
        $.each(stream_channelsA,function(n,v){
            $.each(v,function(m,b){
                var theVal2 = b;
                if(b instanceof Object){
                    theVal2 = JSON.stringify(b);
                }
                var ak= $.aM.e.find('[detail-sw="'+m+'"]')
                $((ak)).val(theVal2).change()
            })
        })
        $.aM.e.find('[detail="switch_stream"]').val(JSON.stringify(stream_channelsA)).change();
    }
    //if not set clear inputs
    $.aM.e.find('[detail-sw]').each(function(n,v){
        v=$(v).attr('detail-sw');
        if(!e.ss.switch_stream){
            $('[detail-sw='+v+']').val('')
        }
    })

    //get btn switch stream
    if(e.ss.btn_switch_stream && e.ss.btn_switch_stream!==''){
        var btn_switch_val
        try{
            btn_switch_val = JSON.parse(e.ss.btn_switch_stream)
        }catch(er){
            btn_switch_val = e.ss.btn_switch_stream;
        }
        $.each(btn_switch_val,function(n,v){
            $.each(v,function(m,b){
                var theVal2 = b;
                if(b instanceof Object){
                    theVal2 = JSON.stringify(b);
                }
                var ak= $.aM.e.find('[detail-sw-btn="'+m+'"]')
                $((ak)).val(theVal2).change()
            })
        })
        $.aM.e.find('[detail="btn_switch_stream"]').val(JSON.stringify(btn_switch_val)).change();
    }
    //if not set clear inputs
    $.aM.e.find('[detail-sw-btn]').each(function(n,v){
        v=$(v).attr('detail-sw-btn');
        if(!e.ss.btn_switch_stream){
            $('[detail-sw-btn='+v+']').val('')
        }
    })
    ...
    try{
        ...
        $.each(['departs'],function(m,b){
            var tmp=''
            tmp+='<div id="treeMonEdit"></div>'
            $('#monitor_groups2').html(tmp)

            var newTreeArray = $.extend(true,{},$user.depart_groups);
            var selectedDeparts=[];
            for(var o in newTreeArray) {
                if(!e.ss[b]){
                    e.ss[b]=[]
                }
                if(e.ss[b].indexOf(o)>-1){
                    selectedDeparts.push(newTreeArray[o].iddep)
                }
            }
            var dataArray23 = [];
            for(var index in newTreeArray) {
                dataArray23.push(newTreeArray[index]);
            }

            var treeMonSetting = simTree({
                el: '#treeMonEdit',
                data: dataArray23,
                linkParent: true,
                check: true,
                ...
                onChange: function (item) {
                    ...
                    $.each( item, function( key, value ) {
                        e.s.push(item[key]['iddep']);
                    });
                    $.aM.e.find('[detail="departs"]').val(JSON.stringify(e.s)).change()
                }
            });
            treeMonSetting.setSelected(selectedDeparts)
            treeMonSetting.expandNode('001')
        })
   ...
   console.log(er)
    ...
    $.each($.ccio.mon,function(n,v){
        if(v.ke === $user.ke){
            tmp += $.ccio.tm('option',{auth_token:$user.auth_token,id:v.mid,name:v.name,ipAddr:v.host},null,$user);
        }
    })
    ...
    if($('#copy_settings').val() === '1'){
        ...
        $.each(copyMonitors,function(n,id){
            ...
            if(id === '$New'){
                ...
                monitor.details.fatal_max_timeOut = e.s.details.fatal_max_timeOut
...
$.aM.f.find('[name="type"]').change(function(e){
    ...
    //update value for btn change stream
    $.aM.e.on('change','[detail-sw-btn]',function(){
        var e={};
        e.e=$.aM.e.find('.btn_ch_str_1')
        e.s=[]
        var data={}
        e.e.each(function(n,v){

            $.each($(v).find('[detail-sw-btn]'),function(m,b){
                data[$(b).attr('detail-sw-btn')]=$(b).val()
            });
            e.s.push(data)
        });
        $.aM.e.find('[detail="btn_switch_stream"]').val(JSON.stringify(e.s)).change()
    })


    //update value for change stream
    $.aM.e.on('change','[detail-sw]',function(){
        var e={};
        e.e=$.aM.e.find('.ch_str_1')
        e.s=[]
        var map={}
        e.e.each(function(n,v){

            $.each($(v).find('[detail-sw]'),function(m,b){
                map[$(b).attr('detail-sw')]=$(b).val()
            });
            e.s.push(map)
        });
        $.aM.e.find('[detail="switch_stream"]').val(JSON.stringify(e.s)).change()
    })
})
