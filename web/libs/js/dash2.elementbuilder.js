...
//                if(!d.filename){d.filename=$.ccio.init('tf',d.time)+'.'+d.ext;}
        ...
        break;
        case 1://monitor icon
            let camDetails=JSON.parse(d.details)
            var departID_temp=JSON.parse(d.details)
            departID_temp = departID_temp.departs.replace(/((\[\s*)|(\s*\]|(\s*\")))/g,'')
            var departID = departID_temp.replace(',',' ')
            d.src=placeholder.getData(placeholder.plcimg({bgcolor:'#b57d00',text:'...'}));
            //tmp+='<div auth="'+user.auth_token+'" mid="'+d.mid+'" ke="'+d.ke+'" title="'+d.mid+' : '+d.name+'" class="monitor_block glM'+d.mid+user.auth_token+' filterDiv '+departID+' col-md-5 001"><img monitor="watch" class="snapshot2" src="/libs/snap/'+d.mid+'.png"><div class="box"><div class="title monitor_name truncate">'+d.name+'</div><div class="list-data"><div class="monitor_mid">'+d.mid+'</div></div>'
            tmp+='<div auth="'+user.auth_token+'" mid="'+d.mid+'" ke="'+d.ke+'" title="'+d.mid+' : '+d.name+'" ' +
                'class="monitor_block glM'+d.mid+user.auth_token+' filterDiv '+departID+' col-md-5 001 ">' +
                '<img monitor="watch" class="snapshot2" src="/libs/snap/'+d.mid+'.png">'
            if(camDetails.control==="1"){
                tmp+='<div class="ptz" ><div title="'+d.mode+'" class="tamLamp" ></div><img monitor="watch" src="/libs/img/icon/ptz.png"><span>'+d.name+'</span></div>'
            }else{
                tmp+='<div class="dome" ><div title="'+d.mode+'" class="tamLamp" ></div><img monitor="watch" src="/libs/img/icon/dome.png"><span>'+d.name+'</span></div>'
            }
            tmp+='<div class="box"><div class="title monitor_name truncate">'+d.name+'</div>' +
                '<div class="list-data"><div class="monitor_mid">'+d.mid+'</div></div>'
            tmp+='<div class="icons text-center">'
            tmp+='<div class="btn-group btn-group-xs display-flx">'
            var buttons = {
                "Pop": {
                    "label": "Pop",
                    "attr": "monitor=\"pop\"",
        ...
        case 2://monitor stream
            ...
            tmp+='<div class="lamp" title="'+k.mode+'"><i class="fa fa-eercast"></i></div><div class="controls"><span style="margin-right:33px" title="'+lang['Currently viewing']+'" ><span class="viewers"></span></span> <a class="btn-xs colorW btn" monitor="mode" mode="record">'+lang['Viewing2']+'</a> <a class="btn-xs btn-danger btn" monitor="mode" mode="start">'+lang['Recording']+'</a><span id="inpStream" ></span><span class="monitor_name">'+d.name+'</span></div></div></div></div>'
            ...
            tmp+='<div><span class="monitor_not_record_copy">, '+lang['Recording FPS']+' : <span class="monitor_fps">'+d.fps+'</span></span></div>';
            tmp+='</div>';
            ...
            $.each(buttons,function(n,v){
                tmp+='<a class="iconStyle hovicon effect-3 sub-a btn btn-'+v.class+'" '+v.attr+' title="'+v.label+'"><i class="fa fa-'+v.icon+'"></i></a>'
            })
        ...
        case'option':
            ...
            if(d.ipAddr !=undefined){
                tmp+='<option auth="'+user.auth_token+'"'+selected+' class="selectT" value="'+d.id+'">'+d.ipAddr+' ___ '+d.name+'___'+d.mid+'</option>'
            }else{
                tmp+='<option auth="'+user.auth_token+'"'+selected+' class="selectT" value="'+d.id+'">'+d.name+'___'+d.mid+'</option>'
            }
        break;
        case'stream-element':
            try{k.d=JSON.parse(d.details);}catch(er){k.d=d.details}
            ...
            if($.ccio.op().jpeg_on===true){
                ...
                switch(k.d.stream_type){
                    case'hls':case'flv':case'mp4':
                        tmp+='<video class="stream-element" id="'+d.mid+'" autoplay></video>';
                    break;
                    case'mjpeg':
        ...
        case 'link-set'://Link Shinobi - 1 set
            ...
            tmp+='</div>';
        break;
        case 'dep-set'://department - 1 set
            if(!d.dep){d.dep=''}
            tmp+='<div class="departmentGroup" links="'+d.dep+'">'
            tmp+='<div class="form-group">';
            tmp+='<div class="mdl-grid divider_dep">';
            tmp+='<input type="hidden" group2="iddep" value="'+d.newID+'">';
            tmp+='<input type="hidden" group2="pid">';
            tmp+='<div class="mdl-cell--4-col"><input class="form-control" id="IDnameDep" group2="namedep" placeholder="'+lang['name_department']+'"></div>';
            tmp+='<div class="mdl-cell--4-col"><div id="tree"></div></div>';
           // tmp+='<div class="mdl-cell--2-col"><button class="btn btn-link addRow" type="button" > add </button></div>';
            tmp+='<div class="pull-right"><a class="btn btn-success btn-xs addRow"><i class="fa fa-plus"></i></a></div>';
            tmp+='</div>';
            tmp+='</div>';
            tmp+='</div>';
        break;
...
//                    },
            ...
            tmp+='<div class="form-group-group  input-map" section id="monSectionMap'+tempID+'">'
            tmp+='  <h4 class="header-title">'+lang["Input"]+' <b>'+lang["Map"]+' : <span class="place">'+d.channel+'</span></b>'
        ...
        case 'stream-channel'://Stream Channel
            ...
            tmp+='<div class="form-group-group  stream-channel" section id="monSectionChannel'+tempID+'">'
            tmp+='  <h4 class="header-title">'+lang["Stream Channel"]+' <span class="place">'+d.channel+'</span>'
            tmp+='  <div class="pull-right"><a class="btn btn-danger btn-xs delete"><i class="fa fa-trash-o"></i></a></div>'
            tmp+='  </h4>'
//                tmp+='      <div class="form-group">'
...
//                tmp+='        </label>'
//                tmp+='      </div>'
            tmp+='<div class="form-group-group " input-mapping="stream_channel-'+d.channel+'">'
            tmp+='<h4 class="header-title">'+lang['Input Feed']
    ...
    switch(x){
        ...
        case 2:
            ...
            try{
                ...
                //for show icon change stream in list icons
                if(JSON.parse(d.details).btnSwitch=="1"){
                    k.e.find('.btn-changStream').show()
                }else{
                    k.e.find('.pad').remove();
                    k.e.find('.btn-changStream').hide()
                }
