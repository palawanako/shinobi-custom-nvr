...
if (config.cron.deleteNoVideoRecursion === undefined)
    config.cron.deleteNoVideoRecursion = false;
...
if (config.databaseType === undefined) {
    config.databaseType = 'mysql';
}
if (config.databaseLogs === undefined) {
    config.databaseLogs = false;
}
if (config.useUTC === undefined) {
    config.useUTC = false;
}
...
if (!config.ip || config.ip === '' || config.ip.indexOf('0.0.0.0') > -1)
    config.ip = 'localhost';
if (!config.videosDir) config.videosDir = __dirname + '/videos/';
if (!config.binDir) {
    config.binDir = __dirname + '/fileBin/';
}
...
if (
    databaseOptions.client === 'sqlite3' &&
    databaseOptions.connection.filename === undefined
) {
...
s.sqlQuery = function (query, values, onMoveOn) {
    ...
    return s.databaseEngine.raw(query, values).asCallback(function (err, r) {
        if (err) {
...
s.nameToTime = function (x) {
    (x = x.replace('.webm', '').replace('.mp4', '').split('T')),
        (x[1] = x[1].replace(/-/g, ':'));
    x = x.join(' ');
    return x;
};
io = require('socket.io-client')('ws://' + config.ip + ':' + config.port, {
...
s.deleteVideo = function (x) {
    s.cx({f: 's.deleteVideo', file: x});
};
//Cron Job
...
s.getVideoDirectory = function (e) {
    ...
    if (e.details.dir && e.details.dir !== '') {
        return (
            s.checkCorrectPathEnding(e.details.dir) + e.ke + '/' + e.id + '/'
        );
    } else {
    ...
    if (keys.length > 0) {
        keys.forEach(function (m, current) {
            ...
            if (b.enabled === '1') {
                ...
                b.sql =
                    'WHERE ke=? AND status != 0 AND details NOT LIKE \'%"archived":"1"%\' AND (' +
                    b.sql.join(' AND ') +
                    ')';

                if (b.sort_by && b.sort_by !== '') {
                    b.sql +=
                        ' ORDER BY `' + b.sort_by + '` ' + b.sort_by_direction;
                }
                if (b.limit && b.limit !== '') {
                    b.sql += ' LIMIT ' + b.limit;
                }
...
s.deleteRowsWithNoVideo = function (v, callback) {
    if (
        ...
        (config.cron.deleteNoVideoRecursion === true ||
            (config.cron.deleteNoVideoRecursion === false &&
                !s.alreadyDeletedRowsWithNoVideosOnStart[v.ke]))
    ) {
        s.alreadyDeletedRowsWithNoVideosOnStart[v.ke] = true;
        es = {};
        s.sqlQuery(
            'SELECT * FROM Videos WHERE ke=? AND status!=0 AND details NOT LIKE \'%"archived":"1"%\' AND time < ?',
            [v.ke, s.sqlDate('10 MINUTE')],
            function (err, evs) {
                if (evs && evs[0]) {
                    es.del = [];
                    es.ar = [v.ke];
...
s.deleteOldLogs = function (v, callback) {
    ...
    if (config.cron.deleteLogs === true && v.d.log_days !== 0) {
        s.sqlQuery(
            'DELETE FROM Logs WHERE ke=? AND `time` < ?',
            [v.ke, s.sqlDate(v.d.log_days + ' DAY')],
            function (err, rrr) {
                callback();
                if (err) return console.error(err);
                if (
                    (rrr.affectedRows && rrr.affectedRows.length > 0) ||
...
s.deleteOldEvents = function (v, callback) {
    ...
    if (config.cron.deleteEvents === true && v.d.event_days !== 0) {
        s.sqlQuery(
            'DELETE FROM Events WHERE ke=? AND `time` < ?',
            [v.ke, s.sqlDate(v.d.event_days + ' DAY')],
            function (err, rrr) {
                callback();
                if (err) return console.error(err);
                if (
...
s.deleteOldFileBins = function (v, callback) {
    if (!v.d.fileBin_days || v.d.fileBin_days == '') {
        v.d.fileBin_days = 10;
    } else {
        v.d.fileBin_days = parseFloat(v.d.fileBin_days);
    }
    if (config.cron.deleteFileBins === true && v.d.fileBin_days !== 0) {
        var fileBinQuery = ' FROM Files WHERE ke=? AND `time` < ?';
        s.sqlQuery(
            'SELECT *' + fileBinQuery,
            [v.ke, s.sqlDate(v.d.fileBin_days + ' DAY')],
            function (err, files) {
                if (files && files[0]) {
                    //delete the files
                    files.forEach(function (file) {
                        fs.unlink(
                            s.getFileBinDirectory(file) + file.name,
                            function (err) {
                    ...
                    s.sqlQuery(
                        'DELETE' + fileBinQuery,
                        [v.ke, v.d.fileBin_days],
                        function (err, rrr) {
                            callback();
                            if (err) return console.error(err);
                            if (
                                (rrr.affectedRows &&
                                    rrr.affectedRows.length > 0) ||
                                config.debugLog === true
                            ) {
                                s.cx({
                                    f: 'deleteFileBins',
...
//insert Data in cleaningmonitors TB
s.insertCleaningInfo = function (v, callback) {
    if (v.length > 0) {
        s.sqlQuery(
            'INSERT INTO cleaningmonitors (mid,ke,max_keep_day,path) VALUES ? ON DUPLICATE KEY UPDATE max_keep_day = VALUES(max_keep_day)',
            [v],
            function (err) {
                if (err) {
                    console.log('E insertCleaningInfo');
                }
            },
        );
    }
...
//stored procedure generate list of videos for delete by renaming file names[_exp.mp4]
s.deleteVideosFromSp = function (callback) {
    const path = '/storage/listsFile.txt';
    ...
    s.sqlQuery('CALL Delete_Video_SP10()', function (err, rows) {
        if (err) {
            console.log('_error_CALL_SP');
        } else {
            lineReader.eachLine('/storage/listsFile.txt', function (line) {
                if (fs.existsSync(line)) {
                    try {
                        fs.renameSync(line, line.replace('.mp4', '_exp.mp4'));
                    } catch (err) {
                        console.log('rename Error :', err);
                    }
                }
            });
            console.log('___succ__DELETE_ALL_FILES');
        }
    });
    callback();
};
...
s.deleteTask = function(priority,limit,sto,callback){
    s.sqlQuery('SELECT * FROM Monitors WHERE priority="'+priority+'" ',function(err,cameras){
        if(cameras && cameras[0]){
            cameras.forEach(function(camera){
                //s.sqlQuery('SELECT mid,ke,ext,time,details FROM Videos WHERE details like "%'+sto+'%" AND mid="'+camera.mid+'"  ORDER BY `time` ASC LIMIT '+limit+' ',function(err,videos){
                pool.query('SELECT mid,ke,ext,time,details FROM Videos WHERE details like "%'+sto+'%" AND mid="'+camera.mid+'"  ORDER BY `time` ASC LIMIT '+limit+' ',function(err,videos){
                    var videosToDelete = []
                    var queryValues = []
                    ...
                    if(videos){
                        videos.forEach(function(video){
                            var ddA=JSON.parse(video.details)
                            video.dir = ddA['dir']+video.ke+'/' +video.mid+'/'+s.moment(video.time)+'.'+video.ext
                            ...
                            if (fs.existsSync(video.dir)) {
                                fs.chmod(video.dir,0o777,function(err){
                                    fs.unlink(video.dir,function(err){
                                        ...
                                        if(videosToDelete.length === completedCheck){
                                            ...
                                            s.sqlQuery('DELETE FROM Videos WHERE  ('+videosToDelete+')',queryValues,function(){
                                                console.log('__Delete_DB_')
                                            })
                                        }
                                    })
                                })
                                ...
                                console.log('__not exist_')
                            }
                        })
                    }else{
                        console.log('___no video',err)
                    }
...
s.cronTam = function () {
    k = {};
    config.addStorage.forEach(function (data) {
        let sto = data['path'] + '/';
        k.cmd = 'LANG=C df ' + sto + " | awk 'FNR == 2 {print $5}'";

        run_shell_command(k.cmd).then((res) => {
            if (parseFloat(res) >= 90) {
                //if(true){
                s.deleteTask('low', 2, sto, function () {
                    s.deleteTask('medium', 1, sto, function () {
                        s.deleteTask('high', 1, sto, function () {});
                    });
                });
            } else {
                return;
...
s.cron = function () {
    ...
    s.sqlQuery(
        'SELECT ke,uid,details,mail FROM Users WHERE details NOT LIKE \'%"sub"%\'',
        function (err, rows) {
            if (err) {
                console.error(err);
            }
...
console.log('TAMEYE : cron.js started');
